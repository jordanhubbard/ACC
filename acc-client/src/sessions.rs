...
impl<'a> SessionsApi<'a> {
    ...
    pub async fn get(&self, key: &str) -> Result<Vec<Value>> {
        let url = self.client.url(&format!("/api/sessions/{}", urlencoding(key)));
        let resp = self.client.http().get(&url).send().await.map_err(Error::Http)?;
    ...
    pub async fn put(&self, key: &str, agent: &str, workspace: &str, messages: &[Value]) -> Result<()> {
        let url = self.client.url(&format!("/api/sessions/{}", urlencoding(key)));
        ...
        let resp = self.client.http().put(&url).json(&body).send().await.map_err(Error::Http)?;
        if !resp.status().is_success() {
            let status = resp.status().as_u16();
            let bytes = resp.bytes().await.map_err(Error::Http)?;
            return Err(Error::from_response(status, &bytes));
    ...
    pub async fn delete(&self, key: &str) -> Result<()> {
        let url = self.client.url(&format!("/api/sessions/{}", urlencoding(key)));
        let resp = self.client.http().delete(&url).send().await.map_err(Error::Http)?;
        if !resp.status().is_success() {
            let status = resp.status().as_u16();
            let bytes = resp.bytes().await.map_err(Error::Http)?;
            return Err(Error::from_response(status, &bytes));
